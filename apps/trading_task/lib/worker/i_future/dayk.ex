defmodule TradingTask.Worker.IFuture.Dayk do
  ...
  def data_handler(data, symbol) do
    data =
      ...
      |> Enum.map(fn({x, index}) -> 
        pre_close =
          if index > 0 do
            data |> Enum.at(index - 1) |> Map.get("c")
          else
            x |> Map.get("c")
          end
        
        %{
          date: Map.get(x, "d") |> Date.from_iso8601!(),
          symbol: symbol,
          open: Map.get(x, "o") |> String.to_float(),
          highest: Map.get(x, "h") |> String.to_float(),
          lowest: Map.get(x, "l") |> String.to_float(),
          close: Map.get(x, "c") |> String.to_float(),
          pre_close: pre_close |> String.to_float(),
          volume: Map.get(x, "v") |> String.to_integer(),
