defmodule TradingTask.Worker.Future.State do
  @moduledoc """
  Exq.enqueue(Exq, "default", TradingTask.Worker.Future.State, ["RB0"])
  """
  ...
  def perform(symbol) do
    ...
    dayk_list = Markets.list_future_dayk(symbol: symbol)
    state_list = Markets.list_future_state(symbol: symbol)
    ...
    |> Enum.map(fn(data_chunk) -> 
      {_num, results} = Repo.insert_all(FutureState, data_chunk, returning: true)
      future_state_id = results |> List.last() |> Map.get(:id)
      Markets.update_future(future, %{future_state_id: future_state_id})
    end)
  end
